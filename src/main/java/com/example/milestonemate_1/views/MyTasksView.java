...
public class MyTasksView implements ViewProvider {
    ...
    public AnchorPane getView() {
        ...
        List<Task> myTasks = FileUtils.getAllTasks().stream()
                .filter(task ->
                        task.getAssignedTo().equalsIgnoreCase(loggedInUser)
                                && (!task.getStatus().equalsIgnoreCase("Completed")
                                && !task.getStatus().equalsIgnoreCase("Approved"))
    ...
    private VBox createTaskCard(Task task) {
        ...
        TextFlow priority = createStyledText("Priority : ", task.getPriority());
        ...
        VBox contentBox = new VBox(5, project, title, description, priority, deadline, status);

        // Add file hyperlinks if attached
        ...
        if (task.getStatus().equalsIgnoreCase("Completed")) {
            ...
            completeBtn.setOnAction(e -> {
                ...
                if (updated) {
                    tasksContainer.getChildren().remove(card); // Remove card from UI
